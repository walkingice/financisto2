...
public class CurrencyTest extends AbstractDbTest {
    ...
    public void test_should_reset_default_flag() {
        Currency c1 = CurrencyBuilder.withDb(em).name("USD").title("Dollar").symbol("$").makeDefault().create();
        assertTrue(em.get(Currency.class, c1.id).isDefault);
        Currency c2 = CurrencyBuilder.withDb(em).name("SGD").title("Singapore Dollar").symbol("S$").makeDefault().create();
    ...
    public void test_should_return_home_currency() {
        CurrencyBuilder.withDb(em).name("USD").title("Dollar").symbol("$").makeDefault().create();
        CurrencyBuilder.withDb(em).name("SGD").title("Singapore Dollar").symbol("S$").makeDefault().create();
        assertEquals("SGD", em.getHomeCurrency().name);
    }

    public void test_should_return_empty_currency_if_home_is_not_set() {
        CurrencyBuilder.withDb(em).name("USD").title("Dollar").symbol("$").create();
    ...
    public void test_should_set_home_currency_if_it_has_not_been_set_and_the_same_currency_is_used_in_all_accounts() {
        CurrencyBuilder.withDb(em).name("USD").title("Dollar").symbol("$").create();
        Currency c = CurrencyBuilder.withDb(em).name("SGD").title("Singapore Dollar").symbol("S$").create();
        AccountBuilder.createDefault(em, c);
